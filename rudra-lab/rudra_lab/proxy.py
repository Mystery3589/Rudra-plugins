...
def _start_mitmproxy(cfg: ProxyConfig, tmpdir: str, blocklist: set) -> Optional[subprocess.Popen]:
    ...
    cmd = ["mitmdump", "--listen-host", "127.0.0.1",
           "--listen-port", str(cfg.port), "--quiet", "--scripts", str(script)]
    if cfg.mode == "capture":
...
def start_proxy(cfg: ProxyConfig) -> Optional[ProxySession]:
    if cfg.mode == "no-internet":
        return None
    backend = cfg.backend or detect_proxy_backend()
    if backend == "none":
        console.print("[yellow]No proxy backend (install mitmproxy or tinyproxy) — falling back to no-internet.[/yellow]")
        return None
