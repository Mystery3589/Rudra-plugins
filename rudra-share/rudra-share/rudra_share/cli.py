...
def default_share(ctx: typer.Context):
    """
    Expose local ports to a public HTTPS URL with zero config.

    \b
    Quick start:
      rudra share port 3000        # expose port 3000
      rudra share port 8080 --qr   # expose port 8080 + show QR code
      rudra share list             # list active tunnels
      rudra share stop 3000        # stop a tunnel
    """
...
def share_port(
    port: Annotated[int, typer.Argument(help="Local port to share publicly (e.g. 3000, 8080).")],
...
def list_cmd():
    ...
    if not active:
        ...
        console.print(f"[{th.dim}]Start one with: [bold]rudra share port 3000[/bold][/{th.dim}]")
        return
...
def qr_cmd(url: Annotated[str, typer.Argument(help="URL or text to render as a terminal QR code.")]):
    ...
    if qr_text:
        ...
        console.print(f"[bold {th.error}]Could not generate QR code.[/bold {th.error}]")
